...
function enterApps() {
  homePage.style.display = 'none';
  innerApp.classList.add('open');
  switchAppsTab(document.getElementById('tab-apps'));
}

function switchAppsTab(btn) {
  closeBrowser();
  document.querySelectorAll('.btab').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
...
// Override switchTab to hide apps section too
...
const _origSwitchTabFilter = switchTabFilter;
function switchTabFilter(tabId, cat, btn) {
  document.getElementById('apps-section').classList.remove('active');
  document.getElementById('games').classList.add('active');
  _origSwitchTabFilter(tabId, cat, btn);
}
